...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
#endif // SSD1306_LCDHEIGHT == 64
#endif // SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH > 96*16
};

#define ssd1306_swap(a, b) { int16_t t = a; a = b; b = t; }

// initializer for I2C
SSD1306::SSD1306(i2c_port_t I2CNUM, int8_t SDA, int8_t SCL, int8_t RST) :
...
void SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  i2c = new I2Cmaster(i2cDevice, sda, scl);
  i2c->reset();
...
void SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
	uint8_t maskpat[8] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
  ...
  const int16_t k = SSD1306_LCDWIDTH/8;
  uint8_t d = y & 0x0007;
  ...
  switch (color)
    {
      case WHITE:   *pointer |=  maskpat[d]; break;
      case BLACK:   *pointer &= ~maskpat[d]; break;
      case INVERSE: *pointer ^=  maskpat[d]; break;
    }
}

void SSD1306::invertDisplay(uint8_t invert) {
  if (invert) {
...
void SSD1306::display(void) {
	...
	ssd1306_command(x_offset);
	ssd1306_command(x_offset + (this->width() - 1));

	ssd1306_command(SSD1306_PAGEADDR);
	ssd1306_command(0x0);
	ssd1306_command((this->height() / 8) - 1);
	...
	uint8_t wrBuffer[18];
    for (uint16_t i=0; i < (SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8);) {
    	wrBuffer[0] = 0x40;
    	memcpy((wrBuffer+1), (buffer + i), 16);
    	i2c->write(i2caddr, wrBuffer, 17);
    	i+=16;
    }
    return;
}
